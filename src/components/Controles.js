...
export const Select = ({ name, className, options, value, descripcion, handlerChange, FirstElement, register }) => 
(    
    <select name={name} class={className} {...register(name)} onChange={handlerChange} >  
       { FirstElement != null && ( <option value={FirstElement.value}> {FirstElement.display} </option> ) }           
        {options.map(option =>(            
